...
must_haves:
  truths:
    - "Navigating to /ksiega-inwentarzowa loads without errors and shows a page with 7 sections"
    - "Sticky left sidebar (desktop) and mobile select show all 7 section nav items"
    - "The 15-column accordion expands each column to show description, tips, and example value"
    - "The procedure checklist persists checked state across page reloads using sessionStorage key 'ksiega_checklist_v1'"
    - "The entry simulator form validates empty fields (red border) only after clicking 'Generuj podgląd'"
    - "The comparison table renders 3 columns: Cecha, Księga Inwentarzowa, Ewidencja cyfrowa"
    - "A dashboard card on DashboardPage links to /ksiega-inwentarzowa"
    - "The route /ksiega-inwentarzowa is wrapped in ProtectedRoute"
  artifacts:
    - path: "src/pages/KsiegaInwentarzPage.jsx"
      provides: "New page with 7 sections, all interactive components, sticky nav, sessionStorage checklist"
      contains: "ksiega_checklist_v1"
    - path: "src/App.jsx"
      provides: "Route entry for /ksiega-inwentarzowa"
      contains: "ksiega-inwentarzowa"
    - path: "src/pages/DashboardPage.jsx"
      provides: "Dashboard card linking to /ksiega-inwentarzowa"
      contains: "ksiega-inwentarzowa"
  key_links:
    - from: "src/App.jsx import"
      to: "src/pages/KsiegaInwentarzPage.jsx"
      via: "import KsiegaInwentarzPage"
      pattern: "import KsiegaInwentarzPage"
    - from: "Checklist component"
      to: "sessionStorage key 'ksiega_checklist_v1'"
      via: "toggleCheck saves JSON to sessionStorage"
      pattern: "ksiega_checklist_v1"
    - from: "entry simulator submit button"
      to: "simSubmitted state"
      via: "setSimSubmitted(true) on click"
      pattern: "simSubmitted"
---

<objective>
Create `src/pages/KsiegaInwentarzPage.jsx` — a self-contained interactive guide to the K-205/60 physical inventory ledger. Add route to App.jsx and dashboard card to DashboardPage.jsx.

Purpose: Requirement INV-01 — SSUEW members need structured guidance on the physical inventory ledger: what it is, its 15 columns, how to make entries, and how it relates to the digital catalog.
Output:
- NEW `src/pages/KsiegaInwentarzPage.jsx` (~600 lines, self-contained)
- MODIFIED `src/App.jsx` (1 import + 1 Route)
- MODIFIED `src/pages/DashboardPage.jsx` (1 Card element)
</objective>
...
@.planning/phases/04-kompendium-ksi-ga-inwentarzowa/04-01-SUMMARY.md
@.planning/phases/04-kompendium-ksi-ga-inwentarzowa/04-02-SUMMARY.md

<interfaces>
<!-- From src/App.jsx — routing pattern to replicate -->
Existing KompendiumPage route (line 146):
```jsx
...
<!-- From src/pages/DashboardPage.jsx — Card component props (lines 150–165) -->
Card signature: `({ to, title, subtitle, icon, colorFrom, colorTo, buttonText })`
Card grid: `<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full">` (line 355)
New card to add inside the grid:
```jsx
<Card
...
<!-- From src/pages/KompendiumPage.jsx — patterns to replicate verbatim in new file -->
Sticky nav layout:
- Breadcrumb: `<div className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-slate-200">`
- Mobile select: `<div className="lg:hidden sticky top-[49px] z-30 bg-white border-b border-slate-200 px-4 py-2.5">`
- Sidebar: `<aside className="hidden lg:block w-[260px] shrink-0 sticky top-[49px] h-[calc(100vh-49px)] overflow-y-auto border-r border-slate-200 bg-white py-6">`
- Main: `<main className="flex-1 min-w-0 p-6 lg:p-10 pb-24">`
- Outer: `<div className="flex max-w-screen-2xl mx-auto">`

sessionStorage Checklist (exact pattern from KompendiumPage lines 213–258):
```jsx
...
<task type="auto">
  <name>Task 1: Create KsiegaInwentarzPage.jsx — full self-contained page</name>
  <read_first>
    - src/pages/KompendiumPage.jsx (lines 1–50 for import pattern and data array structure; lines 208–270 for state declarations and hooks; lines 270–366 for sub-component definitions; lines 368–465 for breadcrumb + mobile select + sidebar layout)
    - .planning/phases/04-kompendium-ksi-ga-inwentarzowa/04-CONTEXT.md (D-10 through D-18 for content specification)
    - .planning/phases/04-kompendium-ksi-ga-inwentarzowa/04-RESEARCH.md (Pattern 2 through Pattern 6, Code Examples section for exact data structures)
  </read_first>
  <files>src/pages/KsiegaInwentarzPage.jsx</files>
  <action>
    Create the file from scratch following the exact self-contained KompendiumPage pattern. The file must have these parts IN ORDER:
    ...
    === PART 3: STATIC DATA ARRAYS ===

    COLUMN_ITEMS (15 entries for the accordion):
    ```jsx
    const COLUMN_ITEMS = [
    ...
    WPIS_CHECKLIST_ITEMS (checklist steps for procedure section):
    ```jsx
    ...
    FAQ_ITEMS (common errors, FAQ style):
    ```jsx
    const FAQ_ITEMS = [
    ...
    ```
    State declarations (in this exact order):
    ```jsx
    ...
    Hooks (identical pattern to KompendiumPage):
    ```jsx
    useEffect(() => {
    ...
    === PART 5: SUB-COMPONENTS (defined as consts inside the component, same as KompendiumPage) ===

    Checklist — IDENTICAL to KompendiumPage but hover uses emerald accent:
    ```jsx
    ...
    KsiegaAccordion — extended variant with description/tips/example (NOT the simple Accordion from KompendiumPage):
    ```jsx
    ...
    SectionTitle — same color-aware version as in Plan 4.1 (copy the full definition):
    ```jsx
    ...
    Outer wrapper: `<div className="min-h-screen bg-slate-50">`

    Breadcrumb bar (sticky, same pattern as KompendiumPage):
    ```jsx
    ...
    Mobile select (sticky at top-[49px]):
    ```jsx
    ...
    Outer flex layout: `<div className="flex max-w-screen-2xl mx-auto">`
    ...
    Main content: `<main className="flex-1 min-w-0 p-6 lg:p-10 pb-24">`
    ...
    After closing `</main>`, close the flex div and add scroll-to-top button (same pattern as KompendiumPage):
    ```jsx
        </main>
      </div>{/* end flex layout */}
  ...
  <verify>grep -n "ksiega_checklist_v1" src/pages/KsiegaInwentarzPage.jsx</verify>
  <acceptance_criteria>
    - `test -f src/pages/KsiegaInwentarzPage.jsx` exits 0
    - `grep -c "ksiega_checklist_v1" src/pages/KsiegaInwentarzPage.jsx` returns at least 2 (state init + toggleCheck save)
    - `grep -n "simSubmitted" src/pages/KsiegaInwentarzPage.jsx` returns at least 3 (state declaration, setter on button click, fieldError function)
    - `grep -c 'id="kol-' src/pages/KsiegaInwentarzPage.jsx` returns exactly 15
    - `grep -n "COMPARISON_ROWS" src/pages/KsiegaInwentarzPage.jsx` returns at least 2 (const declaration + .map() usage)
    - `grep -c 'id="w' src/pages/KsiegaInwentarzPage.jsx` returns at least 15 (WPIS_CHECKLIST_ITEMS entries)
    - `grep -n "TODO: replace with actual photo" src/pages/KsiegaInwentarzPage.jsx` returns exactly 1
    - `grep -n "/ksiega/placeholder.jpg" src/pages/KsiegaInwentarzPage.jsx` returns exactly 1
    - `grep -n "scroll-mt-20" src/pages/KsiegaInwentarzPage.jsx` returns exactly 7 (one per section)
    - `grep -n "KsiegaAccordion" src/pages/KsiegaInwentarzPage.jsx` returns at least 3 (component def + usage in map)
    - `grep -n "top-\[49px\]" src/pages/KsiegaInwentarzPage.jsx` returns at least 2 (sidebar and mobile select)
  </acceptance_criteria>
  <done>KsiegaInwentarzPage.jsx exists with 7 sections, all data arrays, KsiegaAccordion with structured content, Checklist with sessionStorage persistence, entry simulator with deferred validation, comparison table, FAQ cards, sticky nav, scroll-to-top button.</done>
</task>

<task type="auto">
  <name>Task 2: Wire route in App.jsx and add dashboard card in DashboardPage.jsx</name>
  <read_first>
    - src/App.jsx (lines 20–30 for import block; lines 143–150 for the existing /kompendium route location)
    - src/pages/DashboardPage.jsx (lines 354–370 for the Card grid and existing Card entries)
  </read_first>
  <files>src/App.jsx, src/pages/DashboardPage.jsx</files>
  <action>
    === App.jsx ===
    1. Find the import block (lines 20–28). After `import KompendiumPage from './pages/KompendiumPage';`, add:
       ```jsx
       import KsiegaInwentarzPage from './pages/KsiegaInwentarzPage';
       ```

    2. Find the existing route:
       ```jsx
       <Route path="/kompendium" element={<ProtectedRoute><KompendiumPage /></ProtectedRoute>} />
       ```
    ...
    === DashboardPage.jsx ===
    3. Find the Card grid div (line ~355):
       ```jsx
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full animate-slideUp" style={{animationDelay: '0.1s'}}>
       ```
       After the existing Kompendium card:
       ```jsx
       <Card to="/kompendium" icon="📖" title="Kompendium Wiedzy Protokolanta" ... />
       ```
       Add immediately after it (before the Lista Dostępowa card or the isAdmin block):
       ```jsx
       <Card
         to="/ksiega-inwentarzowa"
         icon="📒"
    ...
    Do NOT modify any other routes, imports, or cards. Do NOT change ProtectedRoute definition.
  </action>
  <verify>grep -n "ksiega-inwentarzowa" src/App.jsx src/pages/DashboardPage.jsx</verify>
  <acceptance_criteria>
    - `grep -c "ksiega-inwentarzowa" src/App.jsx` returns at least 2 (import path and Route path prop)
    - `grep -n "KsiegaInwentarzPage" src/App.jsx` returns at least 2 (import statement + JSX usage)
    - `grep -n "ProtectedRoute" src/App.jsx | grep "ksiega"` returns exactly 1 (route is protected)
    - `grep -c "ksiega-inwentarzowa" src/pages/DashboardPage.jsx` returns at least 1 (Card to prop)
    - `grep -n "from-emerald-600" src/pages/DashboardPage.jsx` returns at least 1 (new card color)
    - `grep -n "Przewodnik K-205/60" src/pages/DashboardPage.jsx` returns exactly 1
  </acceptance_criteria>
  <done>App.jsx has import + ProtectedRoute-wrapped route for /ksiega-inwentarzowa; DashboardPage.jsx has new emerald Card entry with correct to/title/subtitle/colorFrom/colorTo/buttonText props.</done>
</task>
...
1. Build check: `cd /c/Users/Mikołaj/Downloads/stand-dashboard-main && npx vite build --mode development 2>&1 | head -30` — must show no JSX/import errors.
2. Route exists: `grep "ksiega-inwentarzowa" src/App.jsx` returns 2 lines.
3. 7 sections exist: `grep -c 'className="scroll-mt-20' src/pages/KsiegaInwentarzPage.jsx` returns 7.
4. Checklist persistence: `grep "ksiega_checklist_v1" src/pages/KsiegaInwentarzPage.jsx` returns 2 lines.
5. No shared components created: `ls src/components/ 2>/dev/null` — directory should not exist (or not contain new files from this phase).
</verification>

<success_criteria>
- KsiegaInwentarzPage.jsx exists with exactly 7 sections, each with scroll-mt-20 (per D-10)
- Placeholder img tag present with TODO comment and path /ksiega/placeholder.jpg (per D-11)
- Checklist uses sessionStorage key 'ksiega_checklist_v1' with group 'wpis' (per D-12)
- KsiegaAccordion renders 15 column items with description/tips/example (per D-13)
- Comparison table renders 6 rows × 3 cols: Cecha / Księga / Ewidencja (per D-14)
- Entry simulator validates fields ONLY after clicking "Generuj podgląd" (simSubmitted gate, per D-15 + pitfall 4)
- Sticky nav: desktop sidebar lg:block + mobile select lg:hidden, top-[49px] (per D-16)
- Route /ksiega-inwentarzowa wrapped in ProtectedRoute in App.jsx (per D-17)
- Dashboard card with colorFrom="from-emerald-600" colorTo="to-green-800" in DashboardPage.jsx (per D-18)
- No src/components/ directory created; no new npm packages
</success_criteria>

<output>
After completion, create `.planning/phases/04-kompendium-ksi-ga-inwentarzowa/04-03-SUMMARY.md`
</output>
