...
const CATEGORY_STYLES = {
  'Uchwały': { icon: '📜', color: 'red', ring: 'ring-red-100', text: 'text-red-700' },
  'Zarządzenia': { icon: '🖊️', color: 'blue', ring: 'ring-blue-100', text: 'text-blue-700' },
  'Szablony': { icon: '📄', color: 'amber', ring: 'ring-amber-100', text: 'text-amber-700' },
  'Instrukcje': { icon: '💡', color: 'sky', ring: 'ring-sky-100', text: 'text-sky-700' },
  'Default': { icon: '⚖️', color: 'slate', ring: 'ring-slate-100', text: 'text-slate-700' }
};
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    const fetchDocs = async () => {
      ...
      try {
        ...
        if (!data.error) {
          // Bajer: Automatycznie sortujemy od najnowszego (na podstawie daty)
  ...
  // Bajer: Dynamicznie wyciągamy kategorie z Excela (brak hardkodowania!)
  const categoriesFromSheets = ['Wszystkie', ...new Set(documents.map(doc => doc.category))];
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 pb-20 pt-24 relative overflow-x-hidden">
      ...
      {/* NAGŁÓWEK */}
      <div className="max-w-5xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-end gap-4 animate-fadeIn">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
            Lex <span className="text-blue-600 tracking-tighter">SSUEW</span>
          
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Uczelniany System Aktów Prawnych zsynchronizowany z chmurą Google.</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden animate-slideUp">
        ...
        {/* LISTA DOKUMENTÓW LUB SKELETON LOADERS */}
        <div className="p-4 min-h-[400px]">
          {isLoading ? (
            // Bajer: Skeleton loaders podczas ładowania z chmury
            ...
            <div className="text-center p-12">
              ...
              <p className="font-bold text-slate-500 text-lg">Brak aktów spełniających kryteria wyszukiwania.</p>
              ...
              {filteredDocs.map(doc => {
                ...
                return (
                  <div 
                    ...
                    className={`group flex flex-col md:flex-row md:items-center gap-4 p-5 rounded-2xl border transition-all cursor-pointer shadow-sm hover:shadow-md ${
                      isObowiazujacy ? 'bg-white border-slate-200 hover:border-blue-300' : 'bg-slate-50 border-slate-200 opacity-75'
                    }`}
                  >
                    
                    {/* Metadane skrócone (Data i Sygnatura) */}
                    <div className="shrink-0 md:w-48 pl-2">
                      ...
                      <p className={`text-sm font-bold ${isObowiazujacy ? style.text : 'text-slate-500'}`}>{doc.signature}</p>
                    </div>

                    {/* Tytuł, Kategoria i Status */}
                    <div className="flex-grow">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        {/* Tag Kategorii z Ikonką */}
                        <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest bg-white ${style.text} px-2 py-0.5 rounded ${style.ring}`}>
                           <span>{style.icon}</span> {doc.category}
                        </span>
                        
                        {/* Pigułka Statusu */}
                        <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${isObowiazujacy ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                          {isObowiazujacy ? '🟢' : '🔴'} {doc.status}
                        </span>
                      </div>
                      ...
                      {/* Relacja "Lex" na liście */}
                      {doc.repeals && doc.repeals !== 'Brak' && isObowiazujacy && (
                        <p className="text-xs text-rose-800 font-medium mt-1">🔄 Uchyla {doc.repeals}</p>
                      )}
                    </div>
      ...
      {/* METRYKA DOKUMENTU (LEX MODAL - Pozostał bez zmian) */}
      {selectedDoc && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-bounceIn overflow-hidden border border-slate-200">
             ...
             <div className="p-4 border-t border-slate-100 bg-slate-50 rounded-b-[2rem] shrink-0">
                <a 
