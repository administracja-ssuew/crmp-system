...
export default function EquipmentPage() {
  ...
  // === SYSTEM REZERWACJI (Dawniej Koszyk) ===
  const [cart, setCart] = useState([]);
  const [isCheckoutOpen, setIsCheckoutOpen] = useState(false);
  
  // Dane do formularza rezerwacji
  ...
  // Funkcja generująca Wniosek o Rezerwację
...
- Dane kontaktowe / Organizacja: ${reservationData.contactInfo}

Proszę o potwierdzenie dostępności sprzętu w tym terminie oraz wyznaczenie godziny odbioru w celu spisania Protokołu Zdawczo-Odbiorczego.
...
Pozdrawiam,`);
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'rented': return <span className="bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">🔵 Aktualnie Wypożyczony</span>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 relative overflow-hidden flex flex-col items-center pb-32">
      ...
      <div className="relative z-10 w-full max-w-6xl pt-16">
        ...
        <div className="bg-white p-2 rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 mb-10 max-w-xl mx-auto flex items-center">
            ...
            <div key={item.id} className={`bg-white rounded-3xl p-6 shadow-lg border transition-all duration-300 group flex flex-col h-full ${isInCart(item.id) ? 'border-indigo-500 shadow-indigo-200 ring-4 ring-indigo-50' : 'border-slate-100 shadow-slate-200/40 hover:shadow-xl hover:-translate-y-1'}`}>
              ...
              <div className="mt-auto pt-6 flex gap-2">
                ...
                <button onClick={() => toggleCart(item)} disabled={item.status === 'maintenance'} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-md transition-all ${isInCart(item.id) ? 'bg-indigo-100 text-indigo-700 border border-indigo-200' : 'bg-indigo-600 hover:bg-indigo-700 text-white disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none'}`}>
                  {isInCart(item.id) ? '✓ Zaznaczono' : '+ Rezerwuj'}
                </button>
              </div>
            </div>
          )) : <div className="col-span-full text-center py-12"><p className="text-slate-400 font-bold text-lg">Brak sprzętu.</p></div>}
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl relative animate-slideUp max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 mb-8">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Data Od</label>
                  <input type="date" value={reservationData.dateFrom} onChange={e => setReservationData({...reservationData, dateFrom: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Data Do</label>
                  <input type="date" value={reservationData.dateTo} onChange={e => setReservationData({...reservationData, dateTo: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cel rezerwacji (Nazwa wydarzenia)</label>
                <input type="text" placeholder="np. Dni Otwarte UEW..." value={reservationData.purpose} onChange={e => setReservationData({...reservationData, purpose: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Organizacja / Osoba Kontaktowa</label>
                <input type="text" placeholder="np. SKN Zarządzania - Jan Kowalski" value={reservationData.contactInfo} onChange={e => setReservationData({...reservationData, contactInfo: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none" />
              </div>
            </div>
            ...
            <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl mb-6">
              ...
              <ul className="text-sm font-bold text-indigo-900 space-y-1">
                {cart.map((item, idx) => <li key={item.id}>{idx + 1}. {item.name}</li>)}
              </ul>
            </div>

            <a href={generateReservationMail()} onClick={() => {setCart([]); setIsCheckoutOpen(false);}} className="block text-center w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl transition-all">
              Wyślij Wniosek do Biura
            </a>
          </div>
        </div>
      )}

      {/* MODAL PASZPORTU (Pozostał bez zmian, gotowy na zgłaszanie usterek aparatem) */}
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fadeIn">
